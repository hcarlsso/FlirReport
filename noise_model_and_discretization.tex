...
voltage.
%If there are several noise sources present in the resistance,
%we assume they can be modelled through a sum of stochastic variables
%\begin{equation}
%  \label{eq:randomvariable_transformation}
...
\begin{align} \label{eq:heat_balance_equation_noise_discr}
 CT_{i+1}&=(\frac{V_b(t_{i})^2}{R(T_{i})}+\varepsilon(P_t+P_s -2A_s \sigma T^4)-G_{leg}(T-T_s))\Delta t + 2 V_b(t) \sigma \sqrt{\Delta t} W \\
 T(0)&=T_s	\nonumber
\end{align}
